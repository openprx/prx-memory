...
fn main() -> io::Result<()> {
    ...
    let server = McpServer::new().map_err(|e| io::Error::new(io::ErrorKind::Other, e))?;
    match mode.as_str() {
        "stdio" => server.serve_stdio(),
        "http" => {
            let addr = std::env::var("PRX_MEMORY_HTTP_ADDR").unwrap_or_else(|_| "127.0.0.1:8787".to_string());
